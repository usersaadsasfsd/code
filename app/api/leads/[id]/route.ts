...
export async function GET(
  ...
  try {
    ...
    console.error('API Error: Failed to fetch lead');
    return NextResponse.json(
...
export async function PUT(
  ...
  try {
    ...
    // If assignedAgent is being set now but was previously empty, set dateAssignedToSales and assignedDate
    if (dataToUpdate.assignedAgent && !existingLead.assignedAgent) {
      ...
      dataToUpdate.dateAssignedToSales = dataToUpdate.dateAssignedToSales ? new Date(dataToUpdate.dateAssignedToSales) : now;
      // Also set assignedDate if not explicitly provided
      if (!dataToUpdate.assignedDate) {
        dataToUpdate.assignedDate = now;
      }
    }

    // Handle date conversions for receivedDate and assignedDate
    ...
    console.error('API Error: Failed to update lead');
    return NextResponse.json(
...
export async function DELETE(
  ...
  try {
    ...
    console.error('API Error: Failed to delete lead');
    return NextResponse.json(
