...
// import { SortOption } from '@/types/sortOption'; // Declare the SortOption variable here
...
export default function HomePage() {
  ...
  // const [sortBy, setSortBy] = useState<SortOption>('created-desc'); // Declare sortBy and setSortBy here
  ...
  const filteredAndSortedLeads = useMemo(() => {
    let filtered = userFilteredLeads.filter(lead => {
      ...
      if (filters.dateRange) {
        const dateFilterType = filters.dateFilterType || 'receivedDate';
        const leadDateRaw = dateFilterType === 'receivedDate' 
          ? (lead.receivedDate || lead.createdAt)
          : lead.createdAt;
        
        // Ensure dates are Date objects (may be strings from state)
        const leadDate = leadDateRaw instanceof Date ? leadDateRaw : new Date(leadDateRaw);
        ...
        if (leadDate && !isNaN(leadDate.getTime())) {
          const leadDateTime = leadDate.getTime();
          const startTime = startDate.getTime();
          const endTime = endDate.getTime();
          if (leadDateTime < startTime || leadDateTime > endTime) {
            return false;
          }
        }
      }
    ...
    // Sort by the selected date filter type (receivedDate or createdAt), newest first
    const dateFilterType = filters.dateFilterType || 'receivedDate';
    filtered.sort((a, b) => {
      const aDateRaw = dateFilterType === 'receivedDate' 
        ? (a.receivedDate || a.createdAt)
        : a.createdAt;
      const bDateRaw = dateFilterType === 'receivedDate' 
        ? (b.receivedDate || b.createdAt)
        : b.createdAt;
      
      const aDate = aDateRaw instanceof Date ? aDateRaw : new Date(aDateRaw);
      const bDate = bDateRaw instanceof Date ? bDateRaw : new Date(bDateRaw);
            ...
            <div className="text-sm text-gray-600">
              ...
              {filters.dateFilterType && (
                <span className="ml-2 text-gray-500">
                  (sorted by {filters.dateFilterType === 'receivedDate' ? 'Received Date' : 'Created Date'}, newest first)
                </span>
              )}
            </div>
          </div>
