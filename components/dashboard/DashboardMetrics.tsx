...
export function DashboardMetrics({ leadTypeFilter }: DashboardMetricsProps) {
  ...
  const filters: ReportFilters = { leadType: leadTypeFilter || 'Lead' };

  const dashboardMetrics = analyticsService.generateDashboardMetrics(leads, user, filters);
  const leadSourceAnalytics = analyticsService.generateLeadSourceAnalytics(leads, filters, user);
