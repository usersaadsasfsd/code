...
export async function subscribeToPush(userId: string): Promise<string | null> {
  ...
  try {
    ...
    if (currentToken) {
      ...
      await sendFcmTokenToBackend(userId, currentToken);
      return currentToken;
    } else {
  ...
  const unsubscribe = onMessage(messaging, (payload: MessagePayload) => {
    ...
    if (transformedPayload.notification && Notification.permission === 'granted') {
      ...
      try {
        ...
        console.error('Error displaying foreground notification');
      }
  ...
  try {
    ...
    if (messaging) {
      await getToken(messaging)
        ...
        .catch(() => {
          // Token may not exist or permission denied
        });
    }
    ...
    console.error('Error unsubscribing from push notifications');
    return false;
  }
}
