...
export class LeadsAPI {
  ...
  static async updateLead(id: string, updateData: Partial<Lead>): Promise<Lead> {
    try {
      ...
      // Fetch existing lead to check for assignment changes
      ...
      // Auto-set assignedDate when agent is first assigned
      if (updateData.assignedAgent && !existingLead.assignedAgent) {
        // Only set assignedDate if not explicitly provided in updateData
        if (finalUpdatePayload.assignedDate === undefined) {
          finalUpdatePayload.assignedDate = now
        }
        // Also set dateAssignedToSales if not provided
        if (finalUpdatePayload.dateAssignedToSales === undefined) {
          finalUpdatePayload.dateAssignedToSales = now
        }
      }
  ...
  static async bulkCreateLeads(
    ...
    console.log(`[LeadsAPI] bulkCreateLeads: ${leadsData.length} records`)
    ...
    for (let i = 0; i < leadsData.length; i++) {
      ...
      try {
        ...
        // Parse receivedDate from multiple possible column names
        ...
        let possibleDateStr: string | undefined
        for (const key of possibleDateKeys) {
          if ((leadData as any)[key]) {
            possibleDateStr = (leadData as any)[key]
            break
          }
        }
        
        if (possibleDateStr) {
          const parsed = parseDDMMYYYY(possibleDateStr)
        ...
        const result = await collection.insertOne({
          ...
          createdAt: now,
          updatedAt: now,
          receivedDate: receivedDateForRow,
